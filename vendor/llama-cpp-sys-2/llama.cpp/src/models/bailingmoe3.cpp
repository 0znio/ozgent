...
void llama_model_bailingmoe3::load_arch_tensors(llama_model_loader & ml) {
    ...
    // Upstream lets the caller decline the MTP block via `ml.load_mtp`. This
    // tree's loader has no such switch — qwen35moe, its only other hybrid MTP
    // arch, loads the block unconditionally — so do the same here. The weights
    // are in the file either way; nothing is obliged to run them.

...
impl<'model> LlamaContext<'model> {
    ...
    #[must_use]
    /// The underlying `llama_context`.
    ///
    /// Needed to reach llama.cpp APIs this crate does not wrap — mtmd
    /// decodes image embeddings straight into the context. The pointer is
    /// valid for as long as this `LlamaContext` lives.

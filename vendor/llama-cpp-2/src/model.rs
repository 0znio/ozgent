...
pub enum RopeType {
    /// Rotary embedding over consecutive dimension pairs.
    Norm,
    /// GPT-NeoX style: the two halves of the head dimension are rotated together.
    NeoX,
    /// Multi-section rotary embedding (text, height, width), as Qwen-VL uses.
    MRope,
    /// Rotary embedding for vision encoders.
    Vision,
}
...
impl LlamaModel {
    ...
    ///
    /// Needed to reach llama.cpp APIs this crate does not wrap — mtmd, the
    /// multimodal library, takes the model handle directly. The pointer is
    /// valid for as long as this `LlamaModel` lives.
    pub fn as_ptr(&self) -> *const llama_cpp_sys_2::llama_model {
        self.model.as_ptr()
    }

    /// Transformer blocks in the model, as the GGUF declares them.
